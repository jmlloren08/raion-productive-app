<?php

namespace App\Actions\Productive\Fetch;

use App\Actions\Productive\AbstractAction;
use App\Actions\Productive\ProcessIncludedData;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Log;

class FetchBills extends AbstractAction
{
    /**
     * Define include relationships for bills
     * 
     * @var array
     */
    protected array $includeRelationships = [
        'purchase_order',
        'creator',
        'deal',
        'attachment'
    ];

    /**
     * Define fallback include relationships if the full set fails
     * 
     * @var array
     */
    protected array $fallbackIncludes = [
        ['purchase_order'], 
        ['creator'],
        ['deal'],
        ['attachment'],
        []  // Empty array means no includes
    ];

    /**
     * Fetch bills from the Productive API
     *
     * @param array $parameters
     * @return array
     */
    public function handle(array $parameters = []): array
    {
        $command = $parameters['command'] ?? null;
        $apiClient = $parameters['apiClient'] ?? null;

        if (!$apiClient) {
            return [
                'success' => false,
                'bills' => [],
                'error' => 'API client is required'
            ];
        }

        try {
            if ($command instanceof Command) {
                $command->info('Fetching bills...');
            }

            $allBills = [];
            $page = 1;
            $pageSize = 100;
            $hasMorePages = true;
            $includeParam = implode(',', $this->includeRelationships);

            while ($hasMorePages) {
                try {
                    // Following Productive API docs for bills
                    $response = $apiClient->get("bills", [
                        'include' => $includeParam,
                        'page' => [
                            'number' => $page,
                            'size' => $pageSize
                        ],
                    ])->throw();

                    if (!$response->successful()) {
                        throw new \Exception('Failed to fetch bills: ' . $response->body());
                    }

                    $responseBody = $response->json();

                    if (!isset($responseBody['data']) || !is_array($responseBody['data'])) {
                        if ($command instanceof Command) {
                            $command->error("Invalid API response format on page {$page}. Missing 'data' array.");
                            $command->warn("Response format: " . json_encode(array_keys($responseBody)));
                        }
                        return [
                            'success' => false,
                            'bills' => [],
                            'error' => "Invalid API response format on page {$page}"
                        ];
                    }

                    $bills = $responseBody['data'];

                    // Process included data if available
                    $processIncludedAction = new ProcessIncludedData();
                    $bills = $processIncludedAction->handle([
                        'responseBody' => $responseBody,
                        'resources' => $bills,
                        'command' => $command
                    ]);

                    $allBills = array_merge($allBills, $bills);

                    // Debug logging for included data
                    if ($command instanceof Command && isset($responseBody['included']) && is_array($responseBody['included'])) {
                        $this->logIncludedData($responseBody['included'], $page, $command);
                    }

                    // Check if we need to fetch more pages
                    if (count($bills) < $pageSize) {
                        $hasMorePages = false;
                    } else {
                        $page++;
                        if ($command instanceof Command) {
                            $command->info("Fetching bills page {$page}...");
                        }
                    }

                    // Log progress
                    if ($command instanceof Command) {
                        $command->info("Fetched " . count($bills) . " bills from page " . ($page - 1));
                    }
                } catch (\Exception $e) {
                    if ($command instanceof Command) {
                        $command->error("Failed to fetch bills page {$page}: " . $e->getMessage());
                    }

                    // If 'include' parameter is causing problems, try with fallback includes
                    if (strpos($e->getMessage(), 'include') !== false) {
                        foreach ($this->fallbackIncludes as $fallbackInclude) {
                            if (implode(',', $fallbackInclude) !== $includeParam) {
                                if ($command instanceof Command) {
                                    $command->warn("Retrying with include parameter: " . ($fallbackInclude ? implode(',', $fallbackInclude) : 'none'));
                                }
                                $includeParam = implode(',', $fallbackInclude);
                                continue 2; // Continue the outer while loop
                            }
                        }
                    }

                    return [
                        'success' => false,
                        'bills' => [],
                        'error' => 'Error fetching bills page ' . $page . ': ' . $e->getMessage()
                    ];
                }
            }

            if ($command instanceof Command) {
                $command->info('Found ' . count($allBills) . ' bills in total');

                // Calculate and log relationship stats
                $relationshipStats = $this->calculateRelationshipStats($allBills);
                $this->logRelationshipStats($relationshipStats, count($allBills), $command);
            }

            return [
                'success' => true,
                'bills' => $allBills
            ];
        } catch (\Exception $e) {
            if ($command instanceof Command) {
                $command->error("Error in bills fetch process: " . $e->getMessage());
            }

            Log::error("Error in bills fetch process: " . $e->getMessage());

            return [
                'success' => false,
                'bills' => [],
                'error' => 'Error in bills fetch process: ' . $e->getMessage()
            ];
        }
    }

    /**
     * Calculate relationship statistics for bills
     *
     * @param array $bills
     * @return array
     */
    protected function calculateRelationshipStats(array $bills): array
    {
        $stats = array_fill_keys($this->includeRelationships, 0);

        foreach ($bills as $bill) {
            if (isset($bill['relationships'])) {
                foreach ($this->includeRelationships as $relationship) {
                    if (
                        isset($bill['relationships'][$relationship]['data']['id']) ||
                        (isset($bill['relationships'][$relationship]['data']) && is_array($bill['relationships'][$relationship]['data']))
                    ) {
                        $stats[$relationship]++;
                    }
                }
            }
        }

        return $stats;
    }

    /**
     * Log relationship statistics to the command output
     *
     * @param array $stats
     * @param int $totalBills
     * @param Command $command
     * @return void
     */
    protected function logRelationshipStats(array $stats, int $totalBills, Command $command): void
    {
        if ($totalBills > 0) {
            foreach ($stats as $relationship => $count) {
                $percentage = round(($count / $totalBills) * 100, 2);
                $command->info("Bills with {$relationship} relationship: {$count} ({$percentage}%)");
            }
        }
    }

    /**
     * Log included data types for debugging
     *
     * @param array $included
     * @param int $page
     * @param Command $command
     * @return void
     */
    protected function logIncludedData(array $included, int $page, Command $command): void
    {
        $includedTypes = [];
        foreach ($included as $include) {
            $type = $include['type'] ?? 'unknown';
            $includedTypes[$type] = ($includedTypes[$type] ?? 0) + 1;
        }
        $command->info("Page {$page} included data: " . json_encode($includedTypes));
    }
} 