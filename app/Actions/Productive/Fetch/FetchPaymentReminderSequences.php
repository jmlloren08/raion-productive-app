<?php

namespace App\Actions\Productive\Fetch;

use App\Actions\Productive\AbstractAction;
use App\Actions\Productive\ProcessIncludedData;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Log;

class FetchPaymentReminderSequences extends AbstractAction
{
    /**
     * Define include relationships for payment reminder sequences
     * 
     * @var array
     */
    protected array $includeRelationships = [
        'creator',
        'updater',
        'payment_reminders',
    ];

    /**
     * Define fallback include relationships if the full set fails
     * 
     * @var array
     */
    protected array $fallbackIncludes = [
        ['creator'],
        ['updater'],
        ['payment_reminders'],
        []  // Empty array means no includes
    ];

    /**
     * Fetch payment reminder sequences from the Productive API
     *
     * @param array $parameters
     * @return array
     */
    public function handle(array $parameters = []): array
    {
        $command = $parameters['command'] ?? null;
        $apiClient = $parameters['apiClient'] ?? null;

        if (!$apiClient) {
            return [
                'success' => false,
                'payment_reminder_sequences' => [],
                'error' => 'Client not provided'
            ];
        }

        try {
            if ($command instanceof Command) {
                $command->info('Fetching payment reminder sequences...');
            }

            $allSequences = [];
            $page = 1;
            $pageSize = 100;
            $hasMorePages = true;
            $includeParam = implode(',', $this->includeRelationships);

            while ($hasMorePages) {
                try {
                    $response = $apiClient->get("payment_reminder_sequences", [
                        'include' => $includeParam,
                        'page' => [
                            'number' => $page,
                            'size' => $pageSize
                        ],
                    ])->throw();

                    if(!$response->successful()) {
                        throw new \Exception("Failed to fetch payment reminder sequences: " . $response->body());
                    }

                    $responseBody = $response->json();

                    if (!isset($responseBody['data']) || !is_array($responseBody['data'])) {
                        if ($command instanceof Command) {
                            $command->error("Invalid API response format on page {$page}. Missing 'data' array.");
                            $command->warn("Response format: " . json_encode(array_keys($responseBody)));
                        }
                        return [
                            'success' => false,
                            'payment_reminder_sequences' => [],
                            'error' => "Invalid API response format on page {$page}"
                        ];
                    }

                    $sequences = $responseBody['data'];

                    // Process included data if available
                    $processIncludedAction = new ProcessIncludedData();
                    $sequences = $processIncludedAction->handle([
                        'responseBody' => $responseBody,
                        'resources' => $sequences,
                        'command' => $command
                    ]);

                    $allSequences = array_merge($allSequences, $sequences);

                    // Debug logging for included data
                    if ($command instanceof Command && isset($responseBody['included']) && is_array($responseBody['included'])) {
                        $this->logIncludedData($responseBody['included'], $page, $command);
                    }

                    // Check if we need to fetch more pages
                    if (count($sequences) < $pageSize) {
                        $hasMorePages = false;
                    } else {
                        $page++;
                        if ($command instanceof Command) {
                            $command->info("Fetching payment reminder sequences page {$page}...");
                        }
                    }

                    if ($command instanceof Command) {
                        $command->info("Fetched " . count($sequences) . " payment reminder sequences from page " . ($page - 1));
                    }
                } catch (\Exception $e) {
                    if ($command instanceof Command) {
                        $command->error("Failed to fetch payment reminder sequences page {$page}: " . $e->getMessage());
                    }

                    // If 'include' parameter is causing problems, try with fallback includes
                    if (strpos($e->getMessage(), 'include') !== false) {
                        foreach ($this->fallbackIncludes as $fallbackInclude) {
                            if (implode(',', $fallbackInclude) !== $includeParam) {
                                if ($command instanceof Command) {
                                    $command->warn("Retrying with include parameter: " . ($fallbackInclude ? implode(',', $fallbackInclude) : 'none'));
                                }
                                $includeParam = implode(',', $fallbackInclude);
                                continue 2; // Continue the outer while loop
                            }
                        }
                    }

                    return [
                        'success' => false,
                        'payment_reminder_sequences' => [],
                        'error' => 'Error fetching payment reminder sequences page ' . $page . ': ' . $e->getMessage()
                    ];
                }
            }

            if ($command instanceof Command) {
                $command->info('Found ' . count($allSequences) . ' payment reminder sequences in total');

                // Calculate and log relationship stats
                $relationshipStats = $this->calculateRelationshipStats($allSequences);
                $this->logRelationshipStats($relationshipStats, count($allSequences), $command);
            }

            return [
                'success' => true,
                'payment_reminder_sequences' => $allSequences
            ];
        } catch (\Exception $e) {
            if ($command instanceof Command) {
                $command->error("Error in payment reminder sequences fetch process: " . $e->getMessage());
            }

            Log::error("Error in payment reminder sequences fetch process: " . $e->getMessage());

            return [
                'success' => false,
                'payment_reminder_sequences' => [],
                'error' => 'Error in payment reminder sequences fetch process: ' . $e->getMessage()
            ];
        }
    }

    /**
     * Calculate relationship statistics for payment reminder sequences
     *
     * @param array $sequences
     * @return array
     */
    protected function calculateRelationshipStats(array $sequences): array
    {
        $stats = array_fill_keys($this->includeRelationships, 0);

        foreach ($sequences as $sequence) {
            if (isset($sequence['relationships'])) {
                foreach ($this->includeRelationships as $relationship) {
                    if (isset($sequence['relationships'][$relationship]['data']['id'])) {
                        $stats[$relationship]++;
                    }
                }
            }
        }

        return $stats;
    }

    /**
     * Log relationship statistics to the command output
     *
     * @param array $stats
     * @param int $totalSequences
     * @param Command $command
     * @return void
     */
    protected function logRelationshipStats(array $stats, int $totalSequences, Command $command): void
    {
        if ($totalSequences > 0) {
            foreach ($stats as $relationship => $count) {
                $percentage = round(($count / $totalSequences) * 100, 2);
                $command->info("Payment reminder sequences with {$relationship} relationship: {$count} ({$percentage}%)");
            }
        }
    }

    /**
     * Log included data types for debugging
     *
     * @param array $included
     * @param int $page
     * @param Command $command
     * @return void
     */
    protected function logIncludedData(array $included, int $page, Command $command): void
    {
        $includedTypes = [];
        foreach ($included as $include) {
            $type = $include['type'] ?? 'unknown';
            $includedTypes[$type] = ($includedTypes[$type] ?? 0) + 1;
        }
        $command->info("Page {$page} included data: " . json_encode($includedTypes));
    }
} 