<?php

namespace App\Actions\Productive\Fetch;

use App\Actions\Productive\AbstractAction;
use App\Actions\Productive\ProcessIncludedData;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\Log;

class FetchTeams extends AbstractAction
{
    /**
     * Define include relationships for teams
     * 
     * @var array
     */
    protected array $includeRelationships = [
        '',
    ];

    /**
     * Define fallback include relationships if the full set fails
     * 
     * @var array
     */
    protected array $fallbackIncludes = [
        []  // Empty array means no includes
    ];

    /**
     * Fetch teams from the Productive API
     *
     * @param array $parameters
     * @return array
     */
    public function handle(array $parameters = []): array
    {
        $command = $parameters['command'] ?? null;
        $apiClient = $parameters['apiClient'] ?? null;

        if (!$apiClient) {
            return [
                'success' => false,
                'teams' => [],
                'error' => 'API client is required'
            ];
        }

        try {
            if ($command instanceof Command) {
                $command->info('Fetching teams...');
            }

            $allTeams = [];
            $page = 1;
            $pageSize = 100;
            $hasMorePages = true;
            $includeParam = implode(',', $this->includeRelationships);

            while ($hasMorePages) {
                try {
                    // Following Productive API docs for teams
                    $response = $apiClient->get("teams", [
                        // 'include' => $includeParam,
                        'page' => [
                            'number' => $page,
                            'size' => $pageSize
                        ],
                        'sort' => 'name'
                    ])->throw();

                    if (!$response->successful()) {
                        throw new \Exception('Failed to fetch teams: ' . $response->body());
                    }

                    $responseBody = $response->json();

                    if (!isset($responseBody['data']) || !is_array($responseBody['data'])) {
                        if ($command instanceof Command) {
                            $command->error("Invalid API response format on page {$page}. Missing 'data' array.");
                            $command->warn("Response format: " . json_encode(array_keys($responseBody)));
                        }
                        return [
                            'success' => false,
                            'teams' => [],
                            'error' => "Invalid API response format on page {$page}"
                        ];
                    }

                    $teams = $responseBody['data'];

                    // Process included data if available
                    $processIncludedAction = new ProcessIncludedData();
                    $teams = $processIncludedAction->handle([
                        'responseBody' => $responseBody,
                        'resources' => $teams,
                        'command' => $command
                    ]);

                    $allTeams = array_merge($allTeams, $teams);

                    // Debug logging for included data
                    if ($command instanceof Command && isset($responseBody['included']) && is_array($responseBody['included'])) {
                        $this->logIncludedData($responseBody['included'], $page, $command);
                    }

                    // Check if we need to fetch more pages
                    if (count($teams) < $pageSize) {
                        $hasMorePages = false;
                    } else {
                        $page++;
                        if ($command instanceof Command) {
                            $command->info("Fetching teams page {$page}...");
                        }
                    }

                    // Log progress
                    if ($command instanceof Command) {
                        $command->info("Fetched " . count($teams) . " teams from page " . ($page - 1));
                    }
                } catch (\Exception $e) {
                    if ($command instanceof Command) {
                        $command->error("Failed to fetch teams page {$page}: " . $e->getMessage());
                    }

                    // If 'include' parameter is causing problems, try with fallback includes
                    if (strpos($e->getMessage(), 'include') !== false) {
                        foreach ($this->fallbackIncludes as $fallbackInclude) {
                            if (implode(',', $fallbackInclude) !== $includeParam) {
                                if ($command instanceof Command) {
                                    $command->warn("Retrying with include parameter: " . ($fallbackInclude ? implode(',', $fallbackInclude) : 'none'));
                                }
                                $includeParam = implode(',', $fallbackInclude);
                                continue 2; // Continue the outer while loop
                            }
                        }
                    }

                    return [
                        'success' => false,
                        'teams' => [],
                        'error' => 'Error fetching teams page ' . $page . ': ' . $e->getMessage()
                    ];
                }
            }

            if ($command instanceof Command) {
                $command->info('Found ' . count($allTeams) . ' teams in total');

                // Calculate and log relationship stats
                $relationshipStats = $this->calculateRelationshipStats($allTeams);
                $this->logRelationshipStats($relationshipStats, count($allTeams), $command);
            }

            return [
                'success' => true,
                'teams' => $allTeams
            ];
        } catch (\Exception $e) {
            if ($command instanceof Command) {
                $command->error("Error in teams fetch process: " . $e->getMessage());
            }

            Log::error("Error in teams fetch process: " . $e->getMessage());

            return [
                'success' => false,
                'teams' => [],
                'error' => 'Error in teams fetch process: ' . $e->getMessage()
            ];
        }
    }

    /**
     * Calculate relationship statistics for teams
     *
     * @param array $teams
     * @return array
     */
    protected function calculateRelationshipStats(array $teams): array
    {
        $stats = array_fill_keys($this->includeRelationships, 0);

        foreach ($teams as $team) {
            if (isset($team['relationships'])) {
                foreach ($this->includeRelationships as $relationship) {
                    if (
                        isset($team['relationships'][$relationship]['data']['id']) ||
                        (isset($team['relationships'][$relationship]['data']) && is_array($team['relationships'][$relationship]['data']))
                    ) {
                        $stats[$relationship]++;
                    }
                }
            }
        }

        return $stats;
    }

    /**
     * Log relationship statistics to the command output
     *
     * @param array $stats
     * @param int $totalTeams
     * @param Command $command
     * @return void
     */
    protected function logRelationshipStats(array $stats, int $totalTeams, Command $command): void
    {
        if ($totalTeams > 0) {
            foreach ($stats as $relationship => $count) {
                $percentage = round(($count / $totalTeams) * 100, 2);
                $command->info("Teams with {$relationship} relationship: {$count} ({$percentage}%)");
            }
        }
    }

    /**
     * Log included data types for debugging
     *
     * @param array $included
     * @param int $page
     * @param Command $command
     * @return void
     */
    protected function logIncludedData(array $included, int $page, Command $command): void
    {
        $includedTypes = [];
        foreach ($included as $include) {
            $type = $include['type'] ?? 'unknown';
            $includedTypes[$type] = ($includedTypes[$type] ?? 0) + 1;
        }
        $command->info("Page {$page} included data: " . json_encode($includedTypes));
    }
} 