...
class SyncProductiveData extends Command
{
    ...
    }private function storeCompany(array $companyData): void
    {
        ...
        $data = [
            ...
            // Add safe fallbacks for all required fields
            'productive_created_at' => $attributes['created_at'] ?? now(),
            'productive_updated_at' => $attributes['updated_at'] ?? now(),
        ];
    ...
    }    private function storeProject(array $projectData): void
    {
        ...
        try {
            ...
            $this->error("Failed to store project '{$projectData['attributes']['name']}' (ID: {$projectData['id']}): " . $e->getMessage());
            // Log additional details for troubleshooting
    ...
    }    private function storeDeal(array $dealData): void
    {
        ...
        try {
            ...
            $this->error("Failed to store deal '{$dealData['attributes']['name']}' (ID: {$dealData['id']}): " . $e->getMessage());
            // Log additional details for troubleshooting
